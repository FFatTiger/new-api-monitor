...
async function PacketSections({ packetPromise }: { packetPromise: Promise<ClickHousePacketResult> }) {
...
export default async function Home({ searchParams }: PageProps) {
  ...
  return <main className="mx-auto flex min-h-screen w-full max-w-[1240px] flex-col gap-8 px-4 py-6 sm:gap-10 sm:px-6 sm:py-8 lg:px-8 lg:py-10">
    ...
    <DashboardRefreshBoundary fallback={<DashboardContentSkeleton />}>
      {packetPromise ? <Suspense key={JSON.stringify(resolved)} fallback={<DashboardContentSkeleton />}><PacketSections packetPromise={packetPromise} /></Suspense>
